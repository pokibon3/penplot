...
class PenPlotterApp:
    ...
    def send_bulk_gcode_commands(self, commands):
        """複数のGCodeコマンドを一括送信（高速化）"""
        try:
            # 複数のコマンドを一つの文字列に結合
            bulk_script = "\n".join([cmd for cmd in commands if cmd.strip() and not cmd.startswith(';')])
            
            url = f"http://{self.klipper_host.get()}:{self.klipper_port.get()}/printer/gcode/script"
            data = {"script": bulk_script}
            response = requests.post(url, json=data, timeout=60)  # タイムアウトを延長
            return response.status_code == 200
        except Exception as e:
            messagebox.showerror("エラー", f"一括コマンド送信に失敗しました:\n{str(e)}")
            return False
    ...
    def execute_plot(self):
        ...
        def plot():
            try:
                # 高速化：バッチサイズでコマンドをグループ化
                batch_size = 50  # 一度に送信するコマンド数
                ...
                for i in range(0, total_lines, batch_size):
                    batch = self.gcode_lines[i:i+batch_size]
                    status_label.config(text=f"バッチ {i//batch_size + 1}/{(total_lines + batch_size - 1)//batch_size} を送信中...")
                    
                    if not self.send_bulk_gcode_commands(batch):
                        messagebox.showerror("エラー", f"バッチコマンド実行に失敗しました")
                        break
                    
                    # 最小限の遅延（削除または大幅短縮）
                    time.sleep(0.01)  # 10msに短縮（元は100ms）
                        
                    progress = (i + batch_size) / total_lines * 100
